	...
	$("#btnNNAPrograma_AgregarNovedad").on("click", function(evento)
	{
		...
		} else
		{
		  ...
		  } else
		  {
		  	...
		  	{
		  		$.post('../server/php/proyecto/nna_AsignarHogar.php', {idUsuario: Usuario.id, idNNA : $("#txtNNAPrograma_id").val(), idMadre : $("#txtNNAMadre_id").val(), idNuevaMadre : 0, Tipo : 1, Fecha : $("#txtNNAPrograma_FechaNovedad").val(), Observaciones: $("#txtNNAPrograma_NObservaciones").val()}, function(data, textStatus, xhr) 
				{
					if (data.Error != "")
					{
						Mensaje("Error", data.Error);
					} else
					{
						...
						//$("#btnNNA_Dellate_Programa").trigger('click');
						//$("#btnNNAPrograma_VolverALaTabla").trigger('click');
